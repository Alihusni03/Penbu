...
import com.bigprojek.penbu.network.ApiConfig
import com.bigprojek.penbu.network.responsemodel.Default
    ...
    private fun Result(){
        ...
        call.enqueue(object : retrofit2.Callback<Default>{
            ...
            override fun onResponse(call: Call<Default>?, response: Response<Default>?) {
                ...
                //Toast.makeText(applicationContext,response?.body()?.nm_bunga,Toast.LENGTH_SHORT).show()

                namaBunga.setText(response?.body()?.nm_bunga)
                namaLain.setText(response?.body()?.nm_Lain)
                musim.setText(response?.body()?.musim)
                warna.setText(response?.body()?.warna)
                asal.setText(response?.body()?.asal)
                if (response?.body()?.nm_bunga.isNullOrBlank()){
                    namaBunga.setText("Jenis Bunga  Belum tersedia")
                    namaLain.setText("null")
                    musim.setText("null")
                    warna.setText("null")
                    asal.setText("null")
                }
